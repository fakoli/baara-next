// tests/validation/suites/api/wasm-queued.spec.ts
//
// Validates the wasm sandbox + queued execution mode matrix.
...
// is probed by attempting to create a wasm task and submitting it; an error or
// a non-completed terminal status indicates the sandbox is not usable on this
// host.
//
...
const definitions = getDefinitionsByCategory("wasm-queued");
...
test.describe("wasm-queued", () => {
  test.beforeAll(async ({ apiClient }) => {
    // Attempt to create and submit a minimal wasm task to the queue.
    ...
    try {
      ...
      const probeExecution = await apiClient.submitTask(probeTask.id);
  ...
  for (const def of easy) {
    test(def.id, async ({ apiClient, metrics }) => {
      ...
      try {
        const t0 = Date.now();
        const execution = await apiClient.submitTask(task.id);
