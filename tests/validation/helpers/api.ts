// tests/validation/helpers/api.ts
//
// Typed API client for BAARA Next validation tests.
// Extended from tests/e2e/helpers/api.ts with two additional methods:
//   - submitTask(taskId): POST /api/tasks/:id/submit → Execution (201)
//   - listExecutionsByTask(taskId): GET /api/executions?taskId=:id → Execution[]
//
...
export function createAPIClient(apiURL: string): APIClient {
  ...
  return {
    ...
    // POST /api/tasks/:id/run — returns the Execution (runs directly, bypasses queue)
    ...
    // POST /api/tasks/:id/submit — submits to queue, returns Execution (201)
    async submitTask(taskId: string): Promise<Execution> {
      return apiFetch<Execution>(`/api/tasks/${taskId}/submit`, {
        method: "POST",
      });
    },
    ...
    // GET /api/executions?taskId=:id — list all executions for a given task
    // Route: packages/server/src/routes/executions.ts line 55-82
    // The query parameter is "taskId" (camelCase as the Hono route reads c.req.query("taskId")).
    async listExecutionsByTask(taskId: string): Promise<Execution[]> {
      return apiFetch<Execution[]>(`/api/executions?taskId=${encodeURIComponent(taskId)}`);
    },
