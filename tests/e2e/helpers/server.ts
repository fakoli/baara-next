...
//
// Server lifecycle helper for BAARA Next E2E tests.
// Spawns a real backend process and a Vite dev server, polls until both are
// ready, and provides a cleanup function that terminates both and removes the
 ...
 *
 * - Creates a fresh temp directory under os.tmpdir() with prefix "baara-e2e-".
...
export async function startServer(): Promise<ServerInstance> {
  ...
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "baara-e2e-"));

  // 2. Pick random ports.
  const backendPort = await getAvailablePort();
  const vitePort = await getAvailablePort();

  // 3. Resolve project root (two levels up from tests/e2e/helpers/).
